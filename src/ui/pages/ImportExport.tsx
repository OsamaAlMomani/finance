  ...
  const exportAllDataZip = async (resetAfter = false) => {
    ...
    try {
      ...
      const [
        ...
        window.electron.invoke('db-get-bills'),
        window.electron.invoke('db-get-loans'),
        window.electron.invoke('db-get-loan-payments', { limit: 100000 }).catch(() => []),
        window.electron.invoke('db-get-metadata', { limit: 100000 }).catch(() => []),
        window.electron.invoke('db-get-realtime-state', { includeExpired: true, limit: 100000 }).catch(() => []),
